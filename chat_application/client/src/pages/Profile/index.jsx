...
const Profile = () => {
  ...
  const validateProfile=()=>{
    if(!firstName)
    {
      toast.error("FirstNmae is required.");
      return false;
    }
    if(!lastName)
    {
  ...
  return (
    <div className='bg-[#1b1c24] h-[100vh] flex items-center justify-center flex-col gap-10 '>
      ...
      <div onClick={()=>handleNavigate()}>
        <IoArrowBack className=" text-4xl lg:text-6xl text-white text-white/90 cursor-pointer"/>
      </div>
      <div className='grid grid-cols-2'>
        <div className='h-full w-32 md:w-48 md:h-48 relative flex items-center jsutify-center' onMouseEnter={()=>setHovered(true)}
          onMouseLeave={()=>setHovered(false)}>
            ...
            {
              hovered && (
                <div className='absolute inset-0 flex items-center justify-center bg-black/50 ring-fuchsia-50 cursor-pointer rounded-full' onClick={image? handleDeleteImage :handleFileInputClick}>
                  {
